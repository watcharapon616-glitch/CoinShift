...
export default function App() {
  ...
  // Bug Fix: Clean up old ObjectURLs to prevent memory leaks
  ...
  const handleConversion = async () => {
    ...
    try {
      ...
      if (targetFormat === 'pdf') {
        ...
        finalResult = pdf.output('datauristring');
      } else {
        finalResult = canvas.toDataURL(`image/${targetFormat === 'png' ? 'png' : 'jpeg'}`, 0.9);
      }
      
      setDownloadUrl(finalResult);
      setIsDone(true);
    } catch (e) {
      ...
      <main className="flex-1 flex flex-col items-center pt-32 pb-20 px-6">
            ...
            {!file ? (
              <label className="group cursor-pointer block">
                ...
                <div className="py-32 flex flex-col items-center text-center px-10">
                  ...
                  <p className="text-sm opacity-50 font-medium max-w-[280px] leading-relaxed">
                    Convert JPG, PNG, HEIC, and PDF documents instantly in your browser.
                  </p>
              ...
              <div className="p-10 space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className={`flex items-center gap-5 p-5 rounded-[2rem] border ${isDark ? 'bg-black/40 border-white/5' : 'bg-slate-50 border-slate-200'}`}>
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-inner ${file.type === 'application/pdf' ? 'bg-rose-500/20 text-rose-500' : 'bg-blue-500/20 text-blue-500'}`}>
                ...
                {!isDone ? (
                  ...
                  <div className="text-center py-10 space-y-10 animate-in zoom-in-95 duration-700">
                    ...
                    <div className="space-y-4 pt-6">
                      <a href={downloadUrl!} download={`PicShift_${targetFormat}_${Date.now()}.${targetFormat}`} className="w-full bg-emerald-600 hover:bg-emerald-500 py-6 rounded-[2rem] text-white font-black text-lg flex items-center justify-center gap-4 shadow-[0_20px_40px_rgba(16,185,129,0.25)] transition-all active:scale-95">
        ...
        <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-3 gap-8 mt-10 border-t border-white/5 pt-20">
          ...
          <div className="space-y-4">
            ...
            <p className="text-sm opacity-50 leading-relaxed">Seamlessly convert between JPG, PNG, and PDF. Full support for modern Apple HEIC files without extra software.</p>
          </div>
          <div className="space-y-4">
        ...
        {/* Brief Article Section */}
        <div className="w-full max-w-2xl mt-32 text-center space-y-6">
          <h2 className="text-4xl font-black italic tracking-tighter uppercase">Professional Image Conversion</h2>
          <p className="text-lg opacity-60 leading-relaxed">
            PicShift is a high-performance web utility designed for photographers, designers, and office workers. 
            By leveraging modern WebAssembly and Canvas technology, we provide a lightning-fast experience 
            without the need for server uploads or subscription fees.
          </p>
        </div>
      </main>
